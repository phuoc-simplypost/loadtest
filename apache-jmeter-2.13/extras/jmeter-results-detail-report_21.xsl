	...
	<html>
		<head>
			...
			<style type="text/css">
				...
				table.failureDetail tr th, table.failureDetail tr td{
					word-wrap: break-word;
					overflow-wrap: break-word;
				}

				table.passedDetail tr th, table.failureDetail tr td{
					word-wrap: break-word;
					overflow-wrap: break-word;
				}
			...
			<tr valign="top">
				<xsl:attribute name="class">
					<xsl:choose>
						...
						<xsl:when test="$failureCount = 0">Passed</xsl:when>
					</xsl:choose>
				</xsl:attribute>
				<td>
...
<xsl:template name="detail">
	...
	<!--<xsl:if test="$allFailureCount > 0">-->
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<xsl:if test="$failureCount > 0">
				<h3 class="Failure"><xsl:value-of select="@lb" /><a><xsl:attribute name="name"><xsl:value-of select="@lb" /></xsl:attribute></a></h3>

				<table align="center" class="details failureDetail" border="0" cellpadding="5" cellspacing="2" width="95%">
				<tr valign="top">
					<th>Response</th>
			...
			<xsl:if test="$failureCount = 0">
				<h3 class="Passed"><xsl:value-of select="@lb" /><a><xsl:attribute name="name"><xsl:value-of select="@lb" /></xsl:attribute></a></h3>

				<table align="center" class="details passedDetail" border="0" cellpadding="5" cellspacing="2" width="95%">
					<tr valign="top">
						<th>Response</th>
						<xsl:if test="$showData = 'y'">
							<th>Request Data</th>
